...
namespace Users.API.Controllers
{
    ...
    [ApiController]
    public class GroupsServiceBaseController : ControllerBase
    {
        private readonly IMediator _mediator;

        public GroupsServiceBaseController(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        {
            // Send a GroupQueryServiceBaseRequest to MediatR, which dispatches it to the appropriate handler (GroupQueryServiceBaseHandler).
            var query = await _mediator.Send(new GroupQueryServiceBaseRequest());
        ...
        {
            // Send a GroupQueryServiceBaseRequest to MediatR, which dispatches it to the appropriate handler (GroupQueryServiceBaseHandler).
            // The handler returns an IQueryable<GroupQueryServiceBaseResponse> representing all groups.
            var query = await _mediator.Send(new GroupQueryServiceBaseRequest());
        ...
        public async Task<IActionResult> Post(GroupCreateServiceBaseRequest request)
        {
            // Check if the incoming request model passes validations through data annotations.
            if (ModelState.IsValid)
            {
                // Send the creation request to MediatR, which will route it to the appropriate handler (GroupCreateServiceBaseHandler).
        ...
        public async Task<IActionResult> Put(GroupUpdateServiceBaseRequest request)
        {
            // Check if the incoming request model passes validations through data annotations.
            if (ModelState.IsValid)
            {
                // Send the update request to MediatR, which will route it to the appropriate handler (GroupUpdateServiceBaseHandler).
                var response = await _mediator.Send(request);
        ...
        {
            // Create a GroupDeleteServiceBaseRequest with the specified group ID and send it to MediatR.
            // MediatR dispatches the request to the appropriate handler (GroupDeleteServiceBaseHandler).
            var response = await _mediator.Send(new GroupDeleteServiceBaseRequest() { Id = id });
